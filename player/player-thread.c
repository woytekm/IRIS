      ...
      switch(G_streams[G_stream_index]->type)
       {
        ...
        case STREAM_TYPE_MP3: // this is also for OGG streams
          G_stream_chan=BASS_StreamCreateURL(G_streams[G_stream_index]->url,0,BASS_SAMPLE_LOOP,0,0);
          break;

        case STREAM_TYPE_AAC:
          G_stream_chan=BASS_AAC_StreamCreateURL(G_streams[G_stream_index]->url,0,BASS_SAMPLE_LOOP,0,0);
          break;

        case STREAM_TYPE_FLAC:
          G_stream_chan=BASS_FLAC_StreamCreateURL(G_streams[G_stream_index]->url,0,BASS_SAMPLE_LOOP,0,0);
          break;
